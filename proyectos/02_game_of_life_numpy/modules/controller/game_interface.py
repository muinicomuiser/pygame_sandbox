...
class GameInterface:
    def __init__(self, config: Config, engine: BaseEngine, recorder: Recorder):
        ...
        self.caption = (
            f"Game of Life - Playing" if self.playing else f"Game of Life - Paused"
        )
    ...
    def _draw_cells_and_grid_np_array_(self):
        ...
        for r, c in zip(rows, columns):
            ...
            pygame.draw.rect(
                self.screen,
                self.config.TILE_COLOR,
                (x, y, self.config.TILESIZE - 2, self.config.TILESIZE - 2),
            )
    ...
    def _draw_cells_np_array_(self):
        ...
        for r, c in zip(rows, columns):
            ...
            pygame.draw.rect(
                self.screen,
                self.config.TILE_COLOR,
                (x, y, self.config.TILESIZE, self.config.TILESIZE),
            )
    ...
    def _render_metrics(self):
        ...
        fps_text = self.font.render(
            f"FPS: {int(self.clock.get_fps())}", True, self.config.FONT_COLOR
        )
        fps_limit_text = self.font.render(
            (
                f"FPS Limit: {int(self.framerate)}"
        ...
        cells_count_text = self.font.render(
            f"Cells count: {alive_count}", True, self.config.FONT_COLOR
        )
        steps_count_text = self.font.render(
            f"Steps count: {self.count}", True, self.config.FONT_COLOR
        )
