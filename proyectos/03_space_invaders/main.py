...
def main():
    ...
    parser = argparse.ArgumentParser(description="Juego Space Invaders")
    parser.add_argument(
        "-r", "--record", help="graba la ejecución", action="store_true"
    )
    parser.add_argument(
        "-f",
        "--fps",
        help="modifica los fps del juego (rango entre 1 y 144)",
        type=int,
        required=False,
    )
